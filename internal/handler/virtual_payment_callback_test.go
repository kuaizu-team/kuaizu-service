...
func TestMatchesVirtualPaymentPrices(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "original unit price and actual line total",
		...
		{
			name:       "reject original line total and actual unit price",
			goods:      virtualPaymentGoodsInfo{OrigPrice: 100, ActualPrice: 10},
			unitPrice:  10,
			totalPrice: 100,
			wantMatch:  false,
		},
		{
			name:       "one unexpected amount",
			goods:      virtualPaymentGoodsInfo{OrigPrice: 10, ActualPrice: 90},
			unitPrice:  10,
			totalPrice: 100,
			wantMatch:  false,
		},
