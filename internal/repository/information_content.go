...
func (r *InformationContentRepository) enrichEventsBatch(ctx context.Context, items []models.InformationContent) error {
	...
	query, args, err := sqlx.In(`SELECT ie.information_id, e.id, e.name, e.is_ranking, e.registration_deadline, e.article_url, e.organizer_name, e.description, e.official_website, e.participation_note, e.resource_url, e.qq_group, e.allow_cross_school, e.allow_cross_major, e.cross_school_major_rule, e.participation_mode, e.team_min_members, e.team_max_members, e.view_count, e.display_order, e.created_at, e.updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&informationID, &event.ID, &event.Name, &event.IsRanking, &event.RegistrationDeadline, &event.ArticleURL, &event.OrganizerName, &event.Description, &event.OfficialWebsite, &event.ParticipationNote, &event.ResourceURL, &event.QQGroup, &event.AllowCrossSchool, &event.AllowCrossMajor, &event.CrossSchoolMajorRule, &event.ParticipationMode, &event.TeamMinMembers, &event.TeamMaxMembers, &event.ViewCount, &event.DisplayOrder, &event.CreatedAt, &event.UpdatedAt); err != nil {
			return err
		}
