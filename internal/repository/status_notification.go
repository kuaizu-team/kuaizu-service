...
func (r *StatusNotificationRepository) GetPending(ctx context.Context, userID int) (*models.StatusNotification, error) {
	const query = `SELECT sn.id, sn.user_id, sn.type, sn.application_id, sn.olive_branch_id, sn.priority,
		COALESCE(pa.project_id, ob.related_project_id) AS project_id,
		p.name AS project_name, COALESCE(pa.applied_at, ob.created_at) AS applied_at,
		...
		COALESCE(ar.name, oar.name) AS assigned_role_name,
		sender.nickname AS operator_name, sn.created_at
		...
		JOIN project p ON p.id = COALESCE(pa.project_id, ob.related_project_id)
